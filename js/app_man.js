        ...
        if (nSet.length === 1 || !mustUseAll) {
            if (currentNumber.value === bestSolution[0] || currentNumber.value === bestSolution[1]) {
                if (challengeStatus) {
                    ...
                    ctn2.empty();
                    ctn2.append(`<a href="whatsapp://send?text=Get ${target} with ${numbersString}? My solution: ${currentNumber.expression} x1F60A http://www.makeanumber.com?${encodeURIComponent(gameId)}" data-action="share/whatsapp/share">Share solution</a>`);
                    $('#winModal').modal('show');
                }
            }
            else {
